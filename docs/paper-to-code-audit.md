...
- `buildPolygonSoupWithAABB()` 合并每坐标的 AABB `floor/ceil` 和量化 `round`，
  ...
  207.446ms）。该方向在 OBJ face 小缓冲后的新基线上再次复测：`ctest --preset default
  --timeout 120` 通过，但两轮 100 组 `run_20260522_110753` / `run_20260522_110902`
  相对 `run_20260522_105412` / `run_20260522_105521` 没有稳定 prepare 收益；
  第二轮相对最新基线 `105521`，overall `prepare_ms` 反而增加 `23.944ms`
  （+0.51%），`end_to_end_ms` 增加 `118.825ms`。该阶段当前不受这一次乘法/finite
  检查主导，合并 helper 的分支和输出参数压力可能抵消了少做缩放的收益；不保留。
...
- 流式叶片分类开始前按 `polygonCount_` 预留 `resultFragments_` 容量，目标是减少每叶输出
  ...
  solve 主导项，不保留。
- WNTV-aware split 在候选选择时复用 center-range 的 split-cost 比较，目标是减少切碎 polygon
  的候选；Debug 构建和 ctest 通过，100 组论文样本 `run_20260522_090251` 的
  `leaf_fragment_count` 小降（34289.62 -> 34158.25）、trace 次数小降（13480.45 ->
...
- OBJ 读取阶段在 tinyobj 解析完成后先统计所有 shape 的 face 数并预留
  `outMesh.faces` 容量，避免大模型读入时 face 外层数组反复扩容；该改动只作用于
  应用 I/O 边界，不改变 face 顺序、索引语义、量化或求解输入。
- 验证：`cmake --preset tests`、`cmake --build --preset tests`、`ctest --preset default
  --timeout 120`、默认 smoke 均通过；small 34 组 oracle 在
  `build/performance/run_20260522_104416/verification.csv` 中 `passed=True` 共 34 项。
- 100 组性能对比：AABB 顶点缓存复用后的两轮保留基线为
  `run_20260522_102820` / `run_20260522_102925`，两次实验为
  `run_20260522_104200` / `run_20260522_104315`。相对 `102925`，overall `read_ms`
  分别下降 `62.554ms`（-3.79%）和 `76.181ms`（-4.62%）；相对 `102820`，
  分别下降 `51.418ms`（-3.14%）和 `65.045ms`（-3.97%）。large 组读入下降
