...
5. 构造 `BoolProblem`，设置布尔运算、输入假设和操作数。
6. 调用 `problem.solve()`。
...
当前实现的核心不是“直接对两个 mesh 做一次布尔”，而是：

1. 先把输入拉进统一整数几何语义。
2. 用 `BoolProblem` 作为外层门面。
3. 用 `SubdivisionSolver` 递归做 AABB 空间细分和参考点传播。
4. 在叶子里先做局部 BSP 编排，再做基于 WNV trace 的片段分类。
5. 只输出布尔指示函数发生内外过渡的有向片段，并保持 OBJ n-gon 导出。
