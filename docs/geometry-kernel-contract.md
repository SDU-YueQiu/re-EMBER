...
- 平面 gcd primitive 化属于导入、公开未信任入口或显式 canonical key 边界；内部 `Trusted` 裁剪路径只接受已 primitive 平面，取反有向平面时保持比例，不再重复 gcd。
- 轴对齐路径段可使用 `AssumePrimitivePlanes` 构造 `Segment256`，因为端点边界来自单位坐标平面或已有 primitive 支撑平面；普通 `Segment256` 构造函数仍保留未信任平面的 primitive 化边界。
