...
Usage:
  ...
  powershell -ExecutionPolicy Bypass -File .\tools\profile-re-ember.ps1 -UsePaperExperimentSet -NoTracy
...
function New-TimingRow {
    ...
    return [pscustomobject]@{
        ...
        leaf_classification_retry_subdivision_count = [math]::Round((Get-Metric $metrics "leaf_classification_retry_subdivision_count"), 0)
...
function Write-Reports {
    ...
    if ($InclusiveZoneRows.Count -gt 0) {
        ...
        $strategyRows.Add((New-StrategySummaryRow $TimingRows $inclusiveAggregates $selfAggregates "leaf_classification_retry_subdivision" "leaf_classification_retry_subdivision_count" @("SubdivisionSolver::retrySubdivisionAfterLeafClassificationPathInvalid")))
