...
`.obj` 输出默认保留 n 边面；`.stl` 输出会在 I/O 边界三角化。默认 polygon soup 输出允许包含论文所述的 T 形连接。

非 raw 的输出拓扑模式整体仍在开发中，不建议用于正常结果。`conforming`、`conforming-merge-convex` 和 `nef` 都只是导出阶段的实验性后处理：不稳定、性能差、已知存在正确性问题，也可能失败或输出误导性的网格。除非是在调试后处理代码本身，否则请保持 `--output-topology raw`。
...
oracle 的精确性边界是 re-EMBER 已经量化后的 `Polygon256` 输入；它不声明验证原始浮点 OBJ/STL 在 CAD 语义上的真实布尔结果。默认缓存目录是 `build\oracle_cache\nef\`；需要强制重算时传 `--refresh-oracle`。`--candidate-mode fragments-nef|export-conforming|export-nef` 可选择用原始结果片段、共形导出拓扑或 Nef 导出拓扑路径构造候选结果；该选项只影响 candidate，不改变 oracle cache key。
...
- `--output-topology raw|conforming|conforming-merge-convex|nef` 选择导出拓扑处理方式。`raw` 是默认值，也是唯一建议使用的模式。其他模式都是开发中的后处理实验：不稳定、性能差、已知存在正确性问题，不适合正常使用。
