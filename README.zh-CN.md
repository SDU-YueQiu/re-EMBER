...
默认支持的本地构建组合是 clang-cl + Boost.Multiprecision。可选的 CGAL oracle 校验工具由 `REEMBER_BUILD_VERIFY` 控制，Nef 导出后处理由 `REEMBER_ENABLE_NEF_POSTPROCESS` 控制，普通本地构建默认都开启。如果缺少 `TBB`、LLVM 或 CGAL，先安装：
...
`.obj` 输出默认保留 n 边面；`.stl` 输出会在 I/O 边界三角化。默认 polygon soup 输出允许包含论文所述的 T 形连接；需要导出共形网格时可传 `--output-topology conforming`，需要进一步保守合并凸共面片段时可传 `--output-topology conforming-merge-convex`。该合并会处理被 T 点切开的连续 subdivision / arrangement 裁剪边链，但仍只合并同向共面且合并后保持凸的相邻片段。

`conforming-merge-convex` 目前只是保守原型，不是工业级共面区域重建；它能修 T-junction 和一部分凸共面片段，但不继续追求完整手写后处理。论文临时展示或需要和 verifier 对接水密正则化网格时，建议传 `--output-postprocess nef`，把结果片段交给 CGAL Nef 正则化后再写 OBJ/STL。这个路径更重，能清理导出拓扑，但不能修复 solver 已经算错的集合结果。
...
oracle 的精确性边界是 re-EMBER 已经量化后的 `Polygon256` 输入；它不声明验证原始浮点 OBJ/STL 在 CAD 语义上的真实布尔结果。默认缓存目录是 `build\oracle_cache\nef\`；需要强制重算时传 `--refresh-oracle`。`--candidate-mode fragments-nef|export-conforming|export-nef` 可选择用原始结果片段、共形导出拓扑或 Nef 后处理路径构造候选结果；该选项只影响 candidate，不改变 oracle cache key。
...
- `--output-topology raw|conforming|conforming-merge-convex` 选择仅作用于导出的拓扑恢复策略。`raw` 为默认值，`conforming` 用精确谓词补齐 T 形连接，`conforming-merge-convex` 还会在合并结果仍为凸多边形时合并相邻共面面，并可跨连续 generated clip 子边链合并。
- `--output-postprocess none|nef` 选择重型导出后处理。`none` 为默认值；`nef` 会把所选导出拓扑交给 CGAL Nef 正则化后再写出网格。
...
- `build\Debug\visual-test.exe` 的 Ember 面板也提供相同的输出拓扑和 Nef 后处理控制。
