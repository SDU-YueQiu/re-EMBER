...
`.obj` 输出默认保留 n 边面；`.stl` 输出会在 I/O 边界三角化。默认 polygon soup 输出允许包含论文所述的 T 形连接；需要导出共形网格时可传 `--output-topology conforming`，需要进一步保守合并凸共面片段时可传 `--output-topology conforming-merge-convex`。
...
- `--output-topology raw|conforming|conforming-merge-convex` 选择仅作用于导出的拓扑恢复策略。`raw` 为默认值，`conforming` 用精确谓词补齐 T 形连接，`conforming-merge-convex` 还会在合并结果仍为凸多边形时合并相邻共面面。
