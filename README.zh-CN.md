...
re-EMBER (redev-open-EMBER) 是一个**独立的社区开源**（MIT）C++17 实现，参考 EMBER 论文 (Trettner et al., SIGGRAPH 2022)。**并非**原论文作者提供的商业授权二进制版本。EMBER 是基于定宽整数几何与局部编排的**精确**网格布尔算法。支持三角面片汤的并集、交集和差集运算，生成水密多边形输出。在所有 benchmark 中，端到端耗时平均仅为 **QuickCSG 的约 2 倍**——而后者不保证输出正确性。当前性能与原论文作者的实现相比仍有**数倍差距**。
...
从 1000 对候选模型中按面数分层抽样：小规模 23 对（1k–5k 面）、中规模 43 对（5k–20k 面）、大规模 34 对（20k–100k 面）。固定二元差运算，叶阈值=25。

| 算法 | 精确 | 中位 | 几何平均 | 最大 | 峰值内存 |
|------|------|------|----------|------|----------|
| **re-EMBER** | 是 | **102 ms** | **113 ms** | 668 ms | 118 MiB |
