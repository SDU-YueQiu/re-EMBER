...
`.obj` 输出保留 n 边面；`.stl` 输出会在 I/O 边界三角化。应用层输出后处理已经禁用：CLI 只导出 raw `BoolProblem::resultFragments()`，不会执行 T-junction 修复、共面合并或 Nef 正则化。
...
- `--output-topology raw` 保持应用层输出为 raw `resultFragments()`；非 raw 输出后处理模式已经禁用。
...
- `build\Debug\visual-test.exe` 的 Ember 面板同样只显示 raw 输出拓扑。
