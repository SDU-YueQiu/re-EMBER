...
re-EMBER 是一个**独立的社区开源**（MIT）C++17 实现，参考 EMBER 论文 (Trettner et al., SIGGRAPH 2022)。**并非**原论文作者提供的商业授权二进制版本。EMBER 是基于定宽整数几何与局部编排的**精确**网格布尔算法。支持三角面片汤的并集、交集和差集运算，生成水密多边形输出。在所有 benchmark 中，端到端耗时平均仅为 **QuickCSG 的约 2 倍**——而后者不保证输出正确性。当前性能与原论文作者的实现相比仍有**数倍差距**。
