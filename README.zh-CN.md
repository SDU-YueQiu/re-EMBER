...
默认支持的本地构建组合是 clang-cl + Boost.Multiprecision。可选的 CGAL oracle 校验工具由 `REEMBER_BUILD_VERIFY` 控制，Nef 导出拓扑由 `REEMBER_ENABLE_NEF_POSTPROCESS` 控制，普通本地构建默认都开启。如果缺少 `TBB`、LLVM 或 CGAL，先安装：
...
`conforming-merge-convex` 目前只是保守原型，不是工业级共面区域重建；它能修 T-junction 和一部分凸共面片段，但不继续追求完整手写后处理。论文临时展示或需要和 verifier 对接水密正则化网格时，建议传 `--output-topology nef`，先补齐 T-junction，再把结果片段交给 CGAL Nef 正则化后写 OBJ/STL。这个路径更重，能清理导出拓扑，但不能修复 solver 已经算错的集合结果。
...
- `--output-topology raw|conforming|conforming-merge-convex|nef` 选择仅作用于导出的拓扑恢复策略。`raw` 为默认值，`conforming` 用精确谓词补齐 T 形连接，`conforming-merge-convex` 还会在合并结果仍为凸多边形时合并相邻共面面，并可跨连续 generated clip 子边链合并，`nef` 则在 T-junction 修复后交给 CGAL Nef 正则化。
...
- `build\Debug\visual-test.exe` 的 Ember 面板也提供相同的输出拓扑控制，包括 `nef`。
